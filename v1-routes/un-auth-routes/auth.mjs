...
import {
    ...
    otpPattern, userModel, otpModelEmail, otpModelPassword
}
    from '../../schema.mjs';
    ...
    try {
        ...
        if (!passwordPattern.test(req?.body?.password)) {
            res.status(400).send({
                ...
                6 to 20 characters and contain at 
                least one alphabet and one number`,
...
router.post(`/send-otp-email`, async (req, res, next) => {
    ...
    if (!emailPattern.test(req?.body?.email)) {
        res.status(400).send({
            message: `invalid email, example request body:
             { "email": "john@example.com" }`,
            errorCode: errorCodes.INVALID_EMAIL
        });
        return;
    ...
    try {
        ...
        // TODO: changge email api to postmark
        await sendEmail(
            user.email,
            user.firstName,
            `Hi ${user.firstName}! Verify your email`,
            `Hi ${user.firstName}! Here is your email 
            verification OTP code. This is 
...
router.post('/verify-email', async (req, res, next) => {
    ...
    try {
        let result = await otpModelEmail
        ...
        const isValid = await
            varifyHash(req?.body?.otpCode, result.otpCodeHash);
        ...
        const isExpired = moment()
            .isAfter(moment(result.createdOn)
                .add(15, 'minutes'));
...
router.post('/login', async (req, res, next) => {
    ...
    if (!passwordPattern.test(req?.body?.password)) {
        res.status(400).send({
            ...
            and contain at least one alphabet and one number`
        })
        return;
    }
...
router.post('/forget-password', async (req, res, next) => {
    ...
    try {
        ...
        // save otp code to database
        const insertResponse = await otpModelPassword
            .create({
                email: req?.body?.email,
                otpCodeHash: otpCodeHash,
            });
        ...
        sendEmail(
            ...
            `Forget password OTP`,
            `Hi ${user.firstName}!
             Here is your forget password OTP code.
             This is valid for 15 minutes: ${otpCode}`
        )
...
router.post('/forget-password-complete', async (req, res, next) => {
    ...
    try {
        ...
        // check if otp code is expired
        const isExpired = moment()
            .isAfter(moment(otpRecord.createdOn)
                .add(15, 'minutes'));
        ...
        const isOtpValid = await
            varifyHash(req?.body?.otpCode, otpRecord.otpCodeHash);
        ...
        // send a response
        res.send({
            message: `forget password completed, 
